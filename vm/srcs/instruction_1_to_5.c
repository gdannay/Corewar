...
int instruction_ld(t_vm *vm, t_process *process)
{
	...
	process->registre[params[1] - 1] = params[0] % IDX_MOD;
	if (!params[0])
		process->carry = 1;
	else
		process->carry = 0;
...
int instruction_sti(t_vm *vm, t_process *process)
{
	...
	if (str[2] == 'r')
		params[1] = process->registre[params[1] - 1];
	write_in_arena_32(vm->arena, process->registre[params[0] - 1], process->position + ((params[1] + params[2]) % IDX_MOD));
	if (!process->registre[params[0] - 1])
		process->carry = 1;
	else
		process->carry = 0;
	process->cycle = 0;
