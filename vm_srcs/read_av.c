...
static	header_t	*check_magic_length(t_player **first,
		header_t *header, char *name)
{
	...
	{
		return (header_error(first, header,
		...
		int fd, char *name, int pl_nbr)
{
	...
	if ((header = check_magic_length(first, header, name)) == NULL)
		return (NULL);
	if ((player = create_player(first, header, pl_nbr)) == NULL)
		return (header_error(first, header, "Malloc failed in file %s", name));
	if (!(int)swap_32_bytes(player->header->prog_size))
	{
		ft_dprintf(2, "Error, champion %sis empty\n", name);
		free_players(first);
		return (NULL);
	}
	return (player);
...
t_player			*read_av(char **av, int ac, int i)
{
	...
	while (++i < ac)
	{
		...
		if ((fd = open(av[i], O_RDONLY)) == -1)
			return (error_read_av(first, "Can't read source file %s\n", av[i]));
